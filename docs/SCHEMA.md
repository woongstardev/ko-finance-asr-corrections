...
- `data/biasing-list.txt` — derived: the verified forms alone, ordered by how badly each is
  misrecognized, for use as ASR contextual-biasing vocabulary. Regenerated by
  `scripts/make_biasing_list.py`; the validator fails if it drifts from `pairs.json`
